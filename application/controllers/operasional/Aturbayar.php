...
class Aturbayar extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        is_logged_in();
        $this->db4 = $this->load->database('operasional', TRUE);
        $this->load->model(array('operasional/Kewajiban_model' => 'Kewajiban_model', 'operasional/Aturbayar_model' => 'Aturbayar_model', 'akademik/Kelas_model' => 'Kelas_model'));
    }
    public function index()
    {
        //echo "Angkatan";
        $data['kontenmenu'] = "Pengaturan";
        $data['kontensubmenu'] = "Kewajiban Mahasiswa";
        $data['aturbayar'] = $this->Aturbayar_model->ambil_data();
        $this->template->display('operasional/aturbayar/index', $data);
    }
    public function akun($id)
    {
        //echo "Angkatan";
        $data['kewajiban'] = $this->Kewajiban_model->ambil_data_id($id);
        $data['kewajiban_id'] = $id;
        $data['kontenmenu'] = "Master Pembukuan";
        $data['kontensubmenu'] = "Akun Kewajiban";
        $data['kodeperkiraan'] = $this->Kodeperkiraan_model->ambil_data();
        $this->template->display('operasional/kewajiban/akun', $data);
    }
    // public function akun($id)
    // {
