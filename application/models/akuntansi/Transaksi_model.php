...
class Transaksi_model extends CI_Model
{
    ...
    {
        $valid = 1;
        return $this->db2->get_where('transaksis', ['nobukti' => $nobukti, 'is_valid' => $valid])->num_rows();
    }
    ...
    public function auditEdit()
    {
        ...
        if ($jurnal == "PM") {
            ...
            $this->db2->query("UPDATE transaksis SET is_valid = 0 WHERE notran='$notran'");
            $this->db3->query("UPDATE operasionals SET is_valid = 0 WHERE notran='$notran'");
        } else {
            // ->result()
            $this->db2->query("insert into audits(transaksi_id,jurnal,tahun_buku,notran,nobukti,noref,tanggal_transaksi,keterangan,accounting,date_created,is_valid,total_transaksi,unit_id,user_id) select * from transaksis where id='$idtran'");
            // $this->db2->delete('transaksis', ['id' => $idtran]);
