...
class Kewajiban_model extends CI_Model
{
    ...
    public function ambil_data()
    {
        ...
        if ($role_id > 1) {
            $status = 1;
            return $this->db4->get_where('kewajibans', ['status' => $status])->result_array();
        } else {
            return $this->db4->get('kewajibans')->result_array();
        }
    }
    public function ambil_data_id($id)
    {
        return $this->db4->get_where('kewajibans', ['id' => $id])->row_array();
    }
