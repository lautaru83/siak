...
class Aturbayar_model extends CI_Model
{
    ...
    public function ambil_data()
    {
        ...
        $institusi_id = $this->session->userdata('institusi_id');
        if ($role_id > 2) {
            //$status = 1;
            //return $this->db4->get_where('detail_dekewajibans', ['status' => $status])->result_array();
            return $this->db4->get('detail_kewajibans')->result_array();
        } else {
            return $this->db4->get('detail_kewajibans')->result_array();
        }
    }
    ...
    {
        return $this->db4->get_where('detail_kewajibans', ['id' => $id])->row_array();
    }
